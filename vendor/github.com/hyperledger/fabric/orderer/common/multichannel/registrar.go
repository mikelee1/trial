...
// Panics on failure.
// TODO 此处严重不同
func ConfigBlock(readWriter blockledger.ReadWriter) *cb.Block {
	lastBlock := blockledger.GetBlock(readWriter, readWriter.Height()-1)
	...
	//尝试从特定文件中读取配置块
	if configBlock, err := readWriter.ReadConfigBlockFromSpecFile(); err == nil && configBlock != nil {
		if configBlock.Header.Number == index {
			logger.Debugf("Got config block from spec file")
			return configBlock
		} else {
			logger.Debugf("The last config index is different from the spec file.")
		}
	} else {
		logger.Debugf("Failed to read config block from file:[%s]", err)
	}

	configBlock := blockledger.GetBlock(readWriter, index)
	if configBlock == nil {
		logger.Panicf("Config block does not exist")
	}
	if err := readWriter.WriteConfigBlockToSpecFile(configBlock); err != nil {
		logger.Errorf("Failed to write config block to file:[%s]", err)
	}

	return configBlock
}

func configTx(readWriter blockledger.ReadWriter) *cb.Envelope {
	return utils.ExtractEnvelopeOrPanic(ConfigBlock(readWriter), 0)
}
