...
var fl *fabric_logs.FabricLogger

// 【日志转储】云象日志orderer模块初始化
func init() {
	fl = fabric_logs.SetFabricLogger("orderer")
...
func initializeBootstrapChannel(genesisBlock *cb.Block, lf blockledger.Factory) {
	...
	if err = gl.WriteConfigBlockToSpecFile(genesisBlock); err != nil {
...
func initializeMultichannelRegistrar(
	...
	// orderer 开启区块归档任务
	if (conf.General.LedgerType == "file" || conf.General.LedgerType == "json") && conf.FileLedger.DataDump.Enabled {
		logger.Debugf("DataDump:{DumpDir:%s,LoadDir:%s,MaxFileLimit:%d,DumpCron:%v,DumpInterval:%d,LoadRetryTimes:%d}",
			conf.FileLedger.DataDump.DumpDir, conf.FileLedger.DataDump.LoadDir, conf.FileLedger.DataDump.MaxFileLimit,
			conf.FileLedger.DataDump.DumpCron, conf.FileLedger.DataDump.DumpInterval, conf.FileLedger.DataDump.LoadRetryTimes)
		go func() {
			cronList := conf.FileLedger.DataDump.DumpCron
			...
			if cronList != nil && len(cronList) > 0 {
				...
				for _, crontab := range cronList {
					...
					err := cronTask.AddFunc(crontab, func() {
						chainIDs := lf.ChainIDs()
						for _, chainId := range chainIDs {
							gl, err := lf.GetOrCreate(chainId)
							if err == nil {
								if dumpErr := gl.DataDump(datadump.DumpForCronTab); err != nil {
									logger.Errorf("Failed to datadump for [%s]", dumpErr)
								}
							}
						}
					})
