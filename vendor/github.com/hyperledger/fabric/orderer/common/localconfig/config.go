...
var Defaults = TopLevel{
	General: General{
		...
		ListenPort:      7050,
		LogPath:         "/var/orderer_log",
		LogName:         "orderer_log",
		maxlinesPerFile: 10000000,
		maxsizePerFile:  10240000,
		maxTotalSize:    409600000,
		isAutoDelete:    false,
		//yun xiang leveldebug
		yxLogLevel:     7,
		maxdays:        7,
		daily:          true,
		rotate:         true,
...
func Load() (*TopLevel, error) {
	...
	if err := initDefaultViper(); err != nil {
		return nil, fmt.Errorf("Error reading configuration for default viper: %s", err)
	}
...
func initDefaultViper() error {
	coreconfig.InitViper(nil, "orderer")
...
func (c *TopLevel) completeInitialization(configDir string) {
	...
	for {
		...
		case c.FileLedger.DataDump.Enabled && c.FileLedger.DataDump.MaxFileLimit == 0:
			logger.Infof("FileLedger.DataDump.MaxFileLimit unset, setting to %d", Defaults.FileLedger.DataDump.MaxFileLimit)
			c.FileLedger.DataDump.MaxFileLimit = Defaults.FileLedger.DataDump.MaxFileLimit
		case c.FileLedger.DataDump.Enabled && c.FileLedger.DataDump.LoadRetryTimes == 0:
			logger.Infof("FileLedger.DataDump.LoadRetryTimes unset, setting to %d", Defaults.FileLedger.DataDump.LoadRetryTimes)
			c.FileLedger.DataDump.LoadRetryTimes = Defaults.FileLedger.DataDump.LoadRetryTimes
		...
		default:
			initDataDumpConfig(c)
			return
		}
	}
}

func initDataDumpConfig(c *TopLevel) {
	if c.FileLedger.DataDump.MaxFileLimit < 0 || c.FileLedger.DataDump.MaxFileLimit > 100000 {
		c.FileLedger.DataDump.MaxFileLimit = 100000
	}
	if c.FileLedger.DataDump.LoadRetryTimes < 0 {
		c.FileLedger.DataDump.LoadRetryTimes = 3
	}
}
