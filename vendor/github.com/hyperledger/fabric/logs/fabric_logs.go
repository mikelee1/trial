...
func SetFabricLogger(containerType string) *FabricLogger {
	once.Do(func() {
		...
		if containerType == "orderer" {
			...
			var err error
			loginfo.isOpenYxlog, err = strconv.ParseBool(config.GetString("general.isOpenYxlog"))
			if err != nil {
				panic(fmt.Sprintf("Error convert general.isOpenYxlog to bool, err: %s", err))
			}
			...
			loglevel, err = strconv.Atoi(config.GetString("general.yxLogLevel"))
			if err != nil {
				panic(fmt.Sprintf("Error convert general.yxLogLevel to int, err: %s", err))
			}
			...
			var err error
			loginfo.isOpenYxlog, err = strconv.ParseBool(config.GetString("logging.isOpenYxlog"))
			if err != nil {
				panic(fmt.Sprintf("Error convert logging.isOpenYxlog to bool, err: %s", err))
			}
			...
			loglevel, err = strconv.Atoi(config.GetString("logging.yxLogLevel"))
			if err != nil {
				panic(fmt.Sprintf("Error convert logging.yxLogLevel to int, err: %s", err))
			}
