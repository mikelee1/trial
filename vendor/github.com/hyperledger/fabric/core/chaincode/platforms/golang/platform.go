...
func (goPlatform *Platform) GenerateDockerBuild(path string, code []byte, tw *tar.Writer) error {
	...
	if factory.GetCurrentBCCSPProvider() == "GM" {
		ldflagsOpt = "-linkmode external "
		cmd = fmt.Sprintf("LD_LIBRARY_PATH=/usr/local/lib CGO_LDFLAGS=\"-I/usr/local/include -L/usr/local/lib\" GOPATH=/chaincode/input:$GOPATH go build -ldflags  \"%s \" -o /chaincode/output/chaincode %s", ldflagsOpt, pkgname)
	}
