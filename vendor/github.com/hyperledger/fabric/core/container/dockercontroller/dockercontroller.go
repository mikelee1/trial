...
// 【日志转储】此处在创建链码容器时将chaincode相关的日志转储配置写入链码容器环境变量，在云象日志模块再从环境变量获取对应的设置
func (vm *DockerVM) createContainer(client dockerClient, imageID, containerID string, args, env []string, attachStdout bool) error {
	...
	if err == nil {
		...
		loginfo.isOpenYxlog = config.GetString("logging.isOpenYxlog")
	...
	isOpenYxLog, _ := strconv.ParseBool(loginfo.isOpenYxlog)
	if isOpenYxLog {
	...
	isOpenYxlogEnv := "CHAINCODE_LOG_ISOPENYXLOG=" + loginfo.isOpenYxlog

	env = append(env, []string{chaincodeLogLevelEnv, logDestinationEnv, maxlinesEnv, maxsizeEnv, maxTotalSizeEnv, isAutoDeleteEnv, dailyEnv, rotateEnv, maxdaysEnv, isOpenYxlogEnv}...)
