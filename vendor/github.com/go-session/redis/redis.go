...
func (s *managerStore) GetKeyValue(_ context.Context, sid string) string {
	value, err := s.getValue(sid)
	if err != nil{
		return ""
	}
...
func (s *store) SaveCustom(sessionid,values string) error {
	...
	cmd := s.ms.cli.Set(sessionid, value, time.Duration(s.expired)*time.Second)
	return cmd.Err()
}
