...
type ManagerStore interface {
	// Check the session store exists, sid是c073b101-ac14-4e77-a8fc-d272a8f7b05b这样形式
	...
	GetKeyValue(ctx context.Context,sid string) string
}
...
type Store interface {
	...
	SaveCustom(sessionid,values string) error
	// Clear all session data
	Flush() error

	DelCustom(sessionid string) error
...
func (s *memoryStore) GetKeyValue( _ context.Context, sid string) string {
	return ""
}
...
func (s *store) SaveCustom(sessionid,values string) error {
	return nil
}


func (s *store) DelCustom(sessionid string) error {
	return nil
}