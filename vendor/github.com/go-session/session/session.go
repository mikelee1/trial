...
func (o *options)GetStore() ManagerStore {
	return o.store
}
...
func (m *Manager)GetOpts() *options {
	return m.opts
}
...
func (m *Manager) sessionID(r *http.Request) (string, error) {
	...
	//mike 三种获取cookievalue的方式
	if m.opts.enableSetCookie {
		fmt.Println("enableSetCookie")
		cookie, err := r.Cookie(m.opts.cookieName)
		if err == nil && cookie.Value != "" {
			cookieValue = cookie.Value
		}
		fmt.Println(m.opts.cookieName,cookie,cookieValue)
	}

	if m.opts.enableSIDInURLQuery && cookieValue == "" {
		fmt.Println("enableSIDInURLQuery")
		err := r.ParseForm()
		if err != nil {
			return "", err
		}
	...
	if m.opts.enableSIDInHTTPHeader && cookieValue == "" {
		fmt.Println("enableSIDInHTTPHeader")
...
func (m *Manager) Start(ctx context.Context, w http.ResponseWriter, r *http.Request) (Store, error) {
	...
	//mike create a sessionid for this session
