...
func FromContext(ctx *gin.Context) session.Store {
	//storeKey is github.com/go-session/gin-session/store
...
func GetManage(ctx *gin.Context) (*session.Manager,error) {
	//manageKey is github.com/go-session/gin-session/manage
	v, ok := ctx.Get(manageKey)
	if ok {
		return v.(*session.Manager),nil
	}
	return &session.Manager{},fmt.Errorf("no manage")
}
